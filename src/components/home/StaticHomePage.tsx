...
const journeySteps = [
  { step: '1', title: 'Book', desc: 'Choose a time that works for you. Appointments are available throughout the day.', icon: CalendarClock },
  { step: '2', title: 'Connect', desc: 'Join your secure video consultation from your smartphone, tablet, or laptop.', icon: Video },
  { step: '3', title: 'Get Care', desc: 'Receive medical advice, prescriptions, or referrals instantly where appropriate.', icon: Stethoscope },
];

const securityFeatures = [
  { title: 'CQC Regulated', desc: 'We meet national standards for safety and quality of digital care delivery.', icon: BadgeCheck },
  { title: 'Data Privacy', desc: 'Medical records are encrypted and managed using strict GDPR-grade controls.', icon: Lock },
  { title: 'Qualified Doctors', desc: 'All clinicians are experienced, registered, and held to high governance standards.', icon: UserCheck },
];

const featuredDigitalCare = [
  { title: 'Video Consultation', desc: 'Face-to-face appointments with Irish Medical Council registered doctors on our secure platform.', cta: 'Book Now', icon: Video },
  { title: 'Digital Prescription', desc: 'Receive prescriptions directly to your local pharmacy after clinical review.', cta: 'Get Prescription', icon: Pill },
  { title: 'Medical Certificates', desc: 'Official medical certs and referral letters delivered quickly through your patient account.', cta: 'Request Certificate', icon: FileText },
];

const digitalCareBenefits = ['Secure Consultations', 'Irish Medical Council GPs', 'Encrypted Records', 'EU Health Compliance'];
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900 font-sans selection:bg-primary selection:text-white overflow-x-hidden">
      <section
        ...
        style={{
          backgroundImage: `linear-gradient(rgba(0, 22, 43, 0.84), rgba(0, 22, 43, 0.68)), url('${heroBgImage.src}')`,
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 w-full">
          <div className="max-w-3xl text-white">
            ...
            <div className="mt-6 sm:mt-8 flex flex-col sm:flex-row sm:flex-wrap gap-3">
              <BookAppointmentLink />
              <Link href="/contact" className="w-full sm:w-auto text-center px-6 sm:px-8 py-3 sm:py-4 border border-white/35 bg-white/10 backdrop-blur-sm rounded-xl font-black hover:bg-white/20 transition-colors">
                How it Works
              </Link>
            </div>
          </div>
        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {appointmentCards.map((item) => (
              <div key={item.time} className={`rounded-xl border p-5 bg-white shadow-sm hover:shadow-md transition-shadow ${item.badge === 'Popular' ? 'border-l-4 border-l-primary border-slate-200' : 'border-slate-200'}`}>
                <div className="flex items-start justify-between">
                  <p className="text-2xl font-black text-primary">{item.time}</p>
                  {item.badge ? <span className={`text-[10px] font-black px-2 py-1 rounded ${item.badge === 'Live' ? 'bg-blue-100 text-blue-700' : 'bg-blue-50 text-blue-600'}`}>{item.badge}</span> : null}
                </div>
      ...
      <section className="py-14 md:py-20 px-4 sm:px-6 bg-slate-50">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-600 mt-3 max-w-2xl mx-auto text-sm sm:text-base">Get medical advice, prescriptions, and certificates from the comfort of your home within minutes.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {featuredDigitalCare.map((service) => (
              <div key={service.title} className="bg-white rounded-xl border border-slate-200 p-7 group hover:border-primary transition-all">
                ...
                {service.title === 'Video Consultation' ? (
                  <BookAppointmentLink className="inline-flex items-center gap-2 mt-6 text-primary font-black text-sm">{service.cta} <ArrowRight className="w-4 h-4" /></BookAppointmentLink>
                ) : (
                  <PatientAreaLink href={service.title === 'Medical Certificates' ? '/dashboard/records' : '/dashboard'} className="inline-flex items-center gap-2 mt-6 text-primary font-black text-sm">{service.cta} <ArrowRight className="w-4 h-4" /></PatientAreaLink>
                )}
              </div>
            ))}
          </div>
          <div className="mt-8 rounded-xl bg-[#0b4da2] text-white px-5 py-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {digitalCareBenefits.map((item) => (
                <div key={item} className="flex items-center gap-2 text-sm"><CheckCircle2 className="w-4 h-4 text-blue-200" /><span>{item}</span></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="py-14 md:py-20 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <h2 className="text-4xl md:text-5xl font-black text-primary text-center mb-16">Your healthcare journey</h2>
      ...
      <section className="py-16 md:py-24 px-4 sm:px-6 bg-slate-100">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-10 md:gap-14">
          <div className="w-full md:w-1/2">
            <Image src={safetyImage} alt="Secure healthcare systems" sizes="(min-width: 768px) 50vw, 100vw" className="w-full h-[280px] sm:h-[360px] md:h-[420px] object-contain rounded-2xl shadow-lg" />
          </div>
          <div className="w-full md:w-1/2">
        ...
        <div className="max-w-7xl mx-auto">
          <div className="rounded-3xl bg-gradient-to-br from-primary to-blue-900 p-7 sm:p-10 md:p-20 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-blue-100 text-base md:text-lg mt-4 max-w-xl mx-auto">Join thousands of patients who trust QuickDoctor for their daily healthcare needs. Available 7 days a week.</p>
              <BookAppointmentLink className="inline-flex mt-8 px-8 sm:px-10 py-3 sm:py-4 bg-primary text-white rounded-xl font-black hover:bg-primary-dark transition-colors">Book Now - €49</BookAppointmentLink>
            </div>
          </div>
        </div>
      </section>
